...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div class="flex items-center justify-between">
            ...
            <div class="bg-green-100 p-3 rounded-full text-green-600"><i class="fas fa-dollar-sign"></i></div>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div class="flex items-center justify-between">
            ...
            <div class="bg-yellow-100 p-3 rounded-full text-yellow-600"><i class="fas fa-clock"></i></div>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div class="flex items-center justify-between">
            ...
            <div class="bg-purple-100 p-3 rounded-full text-purple-600"><i class="fas fa-utensils"></i></div>
        </div>
    </div>
</div>
        ...
        <table class="w-full text-left">
            ...
            <tbody class="text-gray-600 text-sm">
                ...
                <tr class="border-b">
                    ...
                    <td class="py-3 font-semibold">${{ $order->payment->payment_amount ?? 0 }}</td>
                    <td class="py-3">
                        <span class="px-2 py-1 rounded-full text-xs 
                            {{ $order->order_status == 'pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700' }}">
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-center justify-between border-b pb-2">
                <div class="flex items-center">
                    <img src="{{ $item->image_url ? asset('storage/'.$item->image_url) : 'https://via.placeholder.com/40' }}" class="w-10 h-10 rounded mr-3 object-cover">
                    <div>
                        <p class="font-medium">{{ $item->item_name }}</p>
                        <p class="text-xs text-gray-400">{{ $item->category->categoryName ?? 'Uncategorized' }}</p>
                    </div>
                </div>
                <span class="text-blue-600 font-bold">{{ $item->order_items_count }} Sales</span>
            </div>
            @endforeach
        </div>
    </div>
